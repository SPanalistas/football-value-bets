...
class FootballDataClient:
    ...
    def get_today_matches(self) -> list[dict]:
        ...
        data  = self._get("matches", {"dateFrom": today, "dateTo": today})
        all_m = data.get("matches", [])
        return [m for m in all_m if m.get("competition", {}).get("id") in FD_COMPETITIONS]

    def get_team_recent_matches(self, team_id: int, lookback_days: int = 180) -> list[dict]:
        """Last ~10 finished matches for a team (used for stats and form)."""
        ...
        result  = matches[-10:] if len(matches) > 10 else matches
