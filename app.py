...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### 🔍 Debug")
    st.markdown(f"**Mode:** {'Demo' if DEMO else 'Live'}")
    st.markdown(f"**FD key set:** {'✅' if AF_KEY != 'YOUR_API_FOOTBALL_KEY' else '❌'}")
    st.markdown(f"**Odds key set:** {'✅' if ODDS_KEY != 'YOUR_ODDS_API_KEY' else '❌'}")
    if not DEMO:
        if st.button("🧪 Test API call"):
            ...
            try:
                ...
                st.code(f"Status: {r.status_code}\nMatches in response: {len(r.json().get('matches', []))}\nRaw (first 500 chars):\n{r.text[:500]}")
