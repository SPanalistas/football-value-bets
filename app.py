...
from analyzer import (
    APIFootballClient, OddsClient,
    expected_goals, match_probabilities, detect_value_bets, teams_match,
    compute_form_stats, compute_injury_factor, demo_injury,
    LEAGUES, SPORT_KEYS, MIN_GAMES_HOME, MIN_GAMES_AWAY,
    WORLD_CUP_LEAGUE_ID, WORLD_CUP_SEASON, MIN_GAMES_INTL,
    DEMO_FIXTURES, DEMO_TEAM_STATS, DEMO_ODDS, DEMO_FORM,
    current_season,
)
...
# ── API keys ──────────────────────────────────────────────────────────────────
AF_KEY   = os.getenv("API_FOOTBALL_KEY", "YOUR_API_FOOTBALL_KEY")
ODDS_KEY = os.getenv("ODDS_API_KEY",     "YOUR_ODDS_API_KEY")
DEMO     = AF_KEY == "YOUR_API_FOOTBALL_KEY" or ODDS_KEY == "YOUR_ODDS_API_KEY"

if DEMO:
    st.info("ℹ️ **Demo mode** — set env vars `API_FOOTBALL_KEY` and `ODDS_API_KEY` for live data.", icon="ℹ️")
...
def run_analysis():
    ...
    if DEMO:
        fixtures_by_league = DEMO_FIXTURES
        team_stats         = DEMO_TEAM_STATS
        all_odds           = DEMO_ODDS
        af_client          = None
        odds_client        = None
    else:
        af_client          = APIFootballClient(AF_KEY)
        odds_client        = OddsClient(ODDS_KEY)
        fixtures_by_league = {}
        for league_id in LEAGUES:
            season = WORLD_CUP_SEASON if league_id == WORLD_CUP_LEAGUE_ID else base_season
            fixtures_by_league[league_id] = af_client.get_today_fixtures(league_id, season)
        team_stats = {}
        all_odds   = []
    ...
    for league_id, fixtures in fixtures_by_league.items():
        ...
        is_wc       = (league_id == WORLD_CUP_LEAGUE_ID)
        season      = WORLD_CUP_SEASON if is_wc else base_season
        ...
        for fixture in fixtures:
            ...
            home_id   = home["id"]
            away_id   = away["id"]
            kickoff   = fixture["fixture"]["date"][:16].replace("T", " ") + " UTC"
            ...
            if DEMO:
                ...
                home_stats = af_client.get_team_stats(home_id, league_id, season)
                away_stats = af_client.get_team_stats(away_id, league_id, season)
            ...
            # ── Form (last 5 matches)
            if DEMO:
                home_form = DEMO_FORM.get(home_id, {})
                away_form = DEMO_FORM.get(away_id, {})
            else:
                home_form = compute_form_stats(af_client.get_recent_fixtures(home_id), home_id)
                away_form = compute_form_stats(af_client.get_recent_fixtures(away_id), away_id)

            # ── Injuries / suspensions
            if DEMO:
                home_inj = demo_injury(home_id)
                away_inj = demo_injury(away_id)
            else:
                home_inj = compute_injury_factor(af_client.get_injuries(home_id, season))
                away_inj = compute_injury_factor(af_client.get_injuries(away_id, season))
            ...
            results_by_league.setdefault(league_name, []).append({
                "home_team":         home_name,
                "away_team":         away_name,
